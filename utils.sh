...
function json_get() { grep -o "\"${1}\":[^\"]*\"[^\"]*\"" | sed -E 's/".*".*"(.*)"/\1/'; }
function toml_prep() { __TOML__=$(echo "$1" | tr -d '\t\r' | tr "'" '"' | grep -o '^[^#]*' | grep -v '^$' | sed -r 's/(\".*\")|\s*/\1/g; 1i []'); }
function toml_get_table_names() { echo "$__TOML__" | grep -x '\[.*\]' | tr -d '[]' || return 1; }
function toml_get_table() { sed -n "/\[${1}]/,/^\[.*]$/p" <<<"$__TOML__"; }
function toml_get() {
...
function get_prebuilts() {
	...
	RV_INTEGRATIONS_URL=$(gh_req https://api.github.com/repos/${ORG}/revanced-integrations/releases/latest - | json_get 'browser_download_url')
	...
	RV_PATCHES=$(gh_req https://api.github.com/repos/${ORG}/revanced-patches/releases/latest -)
...
function abort() { echo "abort: $1" && exit 1; }

function set_prebuilts() {
...
function get_patch_last_supported_ver() {
	local vs
...
function get_apkmirror_pkg_name() { req "$1" - | sed -n 's;.*id=\(.*\)" class="accent_color.*;\1;p'; }
# ------------------------------

# ------- uptodown -------------
function get_uptodown_resp() { req "${1}/versions" -; }
function get_uptodown_vers() { echo "$1" | grep -x '^[0-9.]* <span>.*</span>' | sed 's/ <s.*//'; }
...
function get_uptodown_pkg_name() {
	local p
...
function uninstall_sh() { echo "${UNINSTALL_SH//__PKGNAME/$1}" >"${2}/uninstall.sh"; }
function customize_sh() {
